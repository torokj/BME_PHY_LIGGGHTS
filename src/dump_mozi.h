...
DumpStyle(mozi,DumpMozi)

#else

#ifndef LMP_DUMP_MOZI_H
#define LMP_DUMP_MOZI_H

#include "dump.h"

namespace LAMMPS_NS {

class DumpMozi : public Dump {
 public:
  DumpMozi(class LAMMPS *, int, char **);
  virtual ~DumpMozi();
 ...
 protected:
  ...
  int btype;			// body type
  int posi;			// position of Pos in buf
  int nc;		     // number of contacts
  int haveforces;	     // do we print out contactforces?
  class ComputePairGranLocal *forces;     // pointer
  double *savebuff;          // save buffer for writing output
  int save_cont_size;	     // size of savecontbuff
  double *savecontbuff;      // save buffer for writing contact output
  int cont_size;	     // size of contbuff
  double *contbuff;          // collecting cont buffer
  int stl_num;			// number of stl files
  char **stl;			// stl namespace
  int wall_num;			// number of wall computes
  class ComputePairGranLocal **wall;	// wall computes
  ...
  typedef void (DumpMozi::*FnPtrHeader)(bigint);
  FnPtrHeader header_choice;           // ptr to write header functions

  typedef int (DumpMozi::*FnPtrConvert)(int, double *);
  FnPtrConvert convert_choice;          // ptr to convert data functions
  int convert_image(int, double *);
  int convert_noimage(int, double *);

  typedef void (DumpMozi::*FnPtrWrite)(int, double *);
  ...
  typedef void (DumpMozi::*FnPtrPack)(int);
...
E: No dump mozi arguments specified

The dump mozi command requires that atom quantities be specified to
output to dump file.

E: Invalid attribute in dump mozi command
...
E: Could not find dump mozi compute ID

The compute ID needed by dump mozi to compute a per-atom quantity
does not exist.

E: Could not find dump mozi fix ID

Self-explanatory.

E: Dump mozi and fix not computed at compatible times

The fix must produce per-atom quantities on timesteps that dump mozi
needs them.

E: Could not find dump mozi variable name

Self-explanatory.

E: Region ID for dump mozi does not exist
...
E: Dump mozi compute does not compute per-atom info

Self-explanatory.

E: Dump mozi compute does not calculate per-atom vector

Self-explanatory.

E: Dump mozi compute does not calculate per-atom array

Self-explanatory.

E: Dump mozi compute vector is accessed out-of-range

Self-explanatory.

E: Dump mozi fix does not compute per-atom info

Self-explanatory.

E: Dump mozi fix does not compute per-atom vector

Self-explanatory.

E: Dump mozi fix does not compute per-atom array

Self-explanatory.

E: Dump mozi fix vector is accessed out-of-range

Self-explanatory.

E: Dump mozi variable is not atom-style variable
